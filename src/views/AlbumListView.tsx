...
export const AlbumListView: React.FC = () => {
  ...
  // Load owned counts for all albums when user is logged in
  useEffect(() => {
    if (!user) return
  ...
  return (
    <div style={{ padding: '16px' }}>
      <h1 style={{ fontSize: 20, fontWeight: 800, marginBottom: 4 }}>World Cup Albums</h1>
    ...
    >
      ...
      {isLoggedIn && (
        <div style={{ display: 'flex', gap: 16, marginTop: 10 }}>
          <Stat label="Owned" value={album.owned} color="#4ade80" />
          <Stat label="Missing" value={album.missing} color="#f87171" />
